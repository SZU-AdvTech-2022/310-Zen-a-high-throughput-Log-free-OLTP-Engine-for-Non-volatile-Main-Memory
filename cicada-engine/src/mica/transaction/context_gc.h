...
void Context<StaticConfig>::gc(bool forced) {
...
#if defined (CWBL)
    context_pmem_persist (const_cast<uint64_t*>(&(gc_info->gc_ts.t2)),
                  sizeof(gc_info->gc_ts.t2));
#endif
...
#if defined (CWBL)
      context_pmem_persist (const_cast<RowVersion<StaticConfig>**>(&(head->older_rv)), 
                    sizeof(head->older_rv));
#endif
...
#if defined (CWBL)
      context_pmem_persist (const_cast<RowVersion<StaticConfig>**>(&(write_rv->older_rv)),
  ...
  // DEBUG
  // printf ("min_rts=%lu before: gc_items_.size()=%lu\n", min_rts.t2, gc_items_.size());
  ...
  // DEBUG
  // printf ("min_rts=%lu after:  gc_items_.size()=%lu\n", min_rts.t2, gc_items_.size());
