...
bool Transaction<StaticConfig>::begin(bool peek_only,
  ...
  if (!ctx_->db_->is_active(ctx_->thread_id_)) {
    // debug
    printf ("thread is not active = %u\n", ctx_->thread_id_);
    return false;
  }
...
bool Transaction<StaticConfig>::check_version() {
  ...
  for (auto i = 0; i < access_size_; i++) {
    ...
    if (rv == nullptr) {
      ...
      // printf ("check version abort reason: table_is_index = %d\n", 
      //   item->tbl->get_table_is_index());
...
#if defined (CWBL)
    ctx_->context_pmem_persist (const_cast<uint64_t*>(&(item->read_rv->rts.t2)), 
                                sizeof(item->read_rv->rts.t2));
#endif
...
#if defined (CWBL)
          ctx_->context_pmem_persist (const_cast<uint64_t*>(&(item->write_rv->wts.t2)),
                                      sizeof(item->write_rv->wts.t2));
          ctx_->context_pmem_persist (const_cast<uint64_t*>(&(item->write_rv->rts.t2)), 
                                      sizeof(item->write_rv->rts.t2));
#endif
        }
...
#if defined (test)
  // write ts, persist it means the transaction committed
  {
    t.switch_to(&Stats::logging);
    if (StaticConfig::kVerbose) printf("logging: ts=%" PRIu64 "\n", ts_.t2);
    if (!ctx_->db_->logger()->log_commit_state (this)) {
      if (StaticConfig::kCollectExtraCommitStats) {
        abort_reason_target_count_ = &ctx_->stats().aborted_by_logging_count;
        abort_reason_target_time_ = &ctx_->stats().aborted_by_logging_time;
      }
...
#if defined (CWBL)  // write behind logging
  {
    t.switch_to(&Stats::logging);
    if (StaticConfig::kVerbose) printf("logging: ts=%" PRIu64 "\n", ts_.t2);
    if (!ctx_->db_->logger()->log(this)) {
...
template <class StaticConfig>
bool PmemTransaction<StaticConfig>::aep_begin (bool peek_only) {
  return Transaction<StaticConfig>::begin(peek_only);
}

template <class StaticConfig>
bool PmemTransaction<StaticConfig>::aep_commit (Result *result) {
  bool status_commit = Transaction<StaticConfig>::commit (result);
  if (status_commit == false) {
    return false;
  }
  // persist aep with special technique
  return aep_commit_item ();
}

template <class StaticConfig>
bool PmemTransaction<StaticConfig>::aep_abort (bool skip_backoff) {
  // debug
  // printf ("PmemTransaction::aep_abort\n");

  bool status_abort = Transaction<StaticConfig>::abort (skip_backoff);
  if (status_abort == false) {
    return false;
  }
  // persist aep with special technique
  return aep_abort_item ();
}

template <class StaticConfig>
bool PmemTransaction<StaticConfig>::aep_commit_item (void) {

  bool mark_at_wset = false;
  bool mark_at_iset = false;
  uint16_t mark_pos = 0;

  for (uint16_t j = 0; j < Transaction<StaticConfig>::wset_size_; j++) {
    auto i = Transaction<StaticConfig>::wset_idx_[j];
    auto item = &Transaction<StaticConfig>::accesses_[i];
    if (item->tbl->get_table_is_index() == true) {
      continue;
    }

    if (item->write_rv->status == RowVersionStatus::kDeleted
        || item->write_rv->status == RowVersionStatus::kInvalid) {
      item->read_rv->aep_rv  = nullptr;  // important

/*    
      // leave it as a lesson, write_rv can be reuse, above is important
      if (item->write_rv->aep_rv) {
        item->tbl->aep_free_row_version (
            item->cf_id, 
            Transaction<StaticConfig>::ctx_->thread_id(), item->write_rv->aep_rv);
      }
      item->write_rv->aep_rv = nullptr;  // wise coin 
                                         // gurantee aep_rv that not in cache can persist at aep
                                         // while older aep_rv can be reuse with dram_rv
*/

      item->head->status = 0xff;
      // item->head->status.store (0xff);
    }
    else if (item->write_rv->status == RowVersionStatus::kCommitted) {

      if (mark_at_wset == false) {  // decide commit mark position
        mark_pos = j;
        mark_at_wset = true;
      }

      // item->head->status = 0x1; done by peek
 
      if (item->write_rv->aep_rv == nullptr) {
        item->write_rv->aep_rv = item->tbl->aep_allocate_row_version (
                                 item->cf_id, Transaction<StaticConfig>::ctx_->thread_id());
        // debug
        // if (item->write_rv->aep_rv == nullptr) {
        //   printf ("aep_commit_item: aep_allocate_row_version error!\n");
        // }
      }
      item->write_rv->aep_rv->wts = item->write_rv->wts;
      item->write_rv->aep_rv->pkey = item->head->pkey;
      item->write_rv->aep_rv->cmark = 0; 

      ::mica::util::memcpy (item->write_rv->aep_rv->data, item->write_rv->data, 
                            item->write_rv->data_size);

      Transaction<StaticConfig>::ctx_->context_pmem_flush (item->write_rv->aep_rv, 
                                       sizeof(PmemRowVersion<StaticConfig>)+item->write_rv->data_size);
    }
  }

  for (uint16_t j = 0; j < Transaction<StaticConfig>::iset_size_; j++) {
    auto i = Transaction<StaticConfig>::iset_idx_[j];
    auto item = &Transaction<StaticConfig>::accesses_[i];

    if (item->tbl->get_table_is_index() == true) continue; 
    if (item->state == RowAccessState::kInvalid) continue;

    if (item->write_rv->status == RowVersionStatus::kCommitted) {

      if (mark_at_wset == false && mark_at_iset == false) {  // decide commit mark position
        mark_pos = j;
        mark_at_iset = true;
      }
      ...
      if (item->write_rv->aep_rv == nullptr) {
        ...
        assert (item->write_rv->aep_rv != nullptr);
      }

      // printf ("item=%p write_rv=%p aep_rv=%p\n", item, item->write_rv, item->write_rv->aep_rv);

      item->write_rv->aep_rv->wts = item->write_rv->wts;
      item->write_rv->aep_rv->pkey = item->head->pkey;
      item->write_rv->aep_rv->cmark = 0; 
 
      ::mica::util::memcpy (item->write_rv->data, item->write_rv->data, 
                            item->write_rv->data_size);
  ...
  /* already done by peek
  // read set need to be mark cahce head 1 when finish
  for (uint16_t j = 0; j < Transaction<StaticConfig>::rset_size_; j++) {
    auto i = Transaction<StaticConfig>::rset_idx_[j];
    auto item = &Transaction<StaticConfig>::accesses_[i];
    item->head->status = 0x1;
  }
  */ 

  // for consistency 
  Transaction<StaticConfig>::ctx_->context_pmem_drain ();

  // when performance, we save N+1 the "1" pmem_persist by use the last access_item_
  // make cmark at most racing rv
  if (mark_at_wset == true) {
    auto i = Transaction<StaticConfig>::wset_idx_[mark_pos];
    auto item = &Transaction<StaticConfig>::accesses_[i];
    item->write_rv->aep_rv->cmark = 1; 
    Transaction<StaticConfig>::ctx_->context_pmem_flush (item->write_rv->aep_rv, sizeof(PmemRowVersion<StaticConfig>));
  }
  else if (mark_at_iset == true) {
    auto i = Transaction<StaticConfig>::iset_idx_[mark_pos];
    auto item = &Transaction<StaticConfig>::accesses_[i];
    item->write_rv->aep_rv->cmark = 1; 
    Transaction<StaticConfig>::ctx_->context_pmem_flush (item->write_rv->aep_rv, sizeof(PmemRowVersion<StaticConfig>));
  }
  ...
  // for recovery constrain pending
  if (Transaction<StaticConfig>::ctx_->thread_id() == 0) {
    Transaction<StaticConfig>::ctx_->db()->ts_ckp_.make_t2_checkpoint 
      (Transaction<StaticConfig>::ts_.t2);

    Transaction<StaticConfig>::ctx_->db()->db_other_persist_.clwb_cnt += 1;
    Transaction<StaticConfig>::ctx_->db()->db_other_persist_.persist_size += sizeof(uint64_t);
  }
...
template <class StaticConfig>
bool PmemTransaction<StaticConfig>::aep_abort_item (void) {
  /* rewrite use iset
  // aer_rv stick to rv, wear leveling at page level
  for (auto j = 0; j< Transaction<StaticConfig>::access_size_; j++) {
    auto item = &Transaction<StaticConfig>::accesses_[j];
    if (item->tbl->get_table_is_index() == true) 
      continue;
    if (item->state == RowAccessState::kNew) {
      ...
      // debug
      // item->write_rv->aep_rv = nullptr;  // important
    }
    else {
      item->head->status = 0x1;
    }
